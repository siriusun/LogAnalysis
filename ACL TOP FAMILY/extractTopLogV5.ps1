...
# --- Configuration ---
$ZipPassword = 'fixmeplease' # Password for 7z extraction, if needed. Leave empty ('') if no password.
# ---------------------

# Check if 7z.exe is available
if (-not (Get-Command 7z.exe -ErrorAction SilentlyContinue)) {
    Write-Host "Error: 7z.exe not found in PATH. Please install 7-Zip and ensure it's added to your system's environment variables." -ForegroundColor Red
...
function Parse-DbxFileName {
    ...
    # Try Format 2 first: DBX_15120236_2025-03-01_13-00-03_-480
    if ($nameParts[0] -eq "DBX" -and $nameParts.Length -ge 4) {
        $sn = $nameParts[1]
        $dateTime = $nameParts[2] # Keep the date part only for consistency, time is in part 3 if needed
    }
    # Try Format 1: CHINA_ACLTOP_700LAS_15120236_00000280000_DBX_15120236_2025-03-01_13-00-03_-480
    elseif ($FileName -match "_DBX_" -and $nameParts.Length -ge 7 ) {
        # Split by _DBX_ first to reliably get the part after it
        $suffixParts = $FileName.Split("_DBX_")[1].Split("_")
        if ($suffixParts.Length -ge 2) {
            $sn = $suffixParts[0]
            $dateTime = $suffixParts[1] # Keep the date part only
        }
    }
    # Try Format 1 variation (assuming SA log format might be slightly different, e.g. different prefix parts count)
    # Example: SITEAGENT_15120236_00000280000_DBX_15120236_2025-03-01_13-00-03_-480 (less parts before DBX)
    elseif ($FileName -match "_DBX_" ) {
        $suffixParts = $FileName.Split("_DBX_")[1].Split("_")
        if ($suffixParts.Length -ge 2) {
            $sn = $suffixParts[0]
            $dateTime = $suffixParts[1]
        }
    }


    if ($null -eq $sn -or $null -eq $dateTime) {
        Write-Host "Warning: Could not parse SN and DateTime from filename: $FileName" -ForegroundColor Yellow
        return $null
    }
    ...
    return [PSCustomObject]@{
        SN               = $sn
        DateTime         = $dateTime # Return only the date part YYYY-MM-DD
...
function Invoke-LogExtraction {
    ...
    foreach ($toplog_DBX in $toplog_DBXs) {
        ...
        $fileInfo = Parse-DbxFileName -FileName $toplog_DBX.Name
        
        if ($null -eq $fileInfo) {
            Write-Host "Unrecognized filename format or parsing error for: $($toplog_DBX.Name), skipping..." -ForegroundColor Yellow
            continue
        }
    ...
    # Create an InitialSessionState to include our custom function
    $sessionState = [System.Management.Automation.Runspaces.InitialSessionState]::CreateDefault()
    # Get the function definition string
    $parseFuncDef = (Get-Command Parse-DbxFileName).Definition
    # Add the function definition to the session state for the runspaces
    $sessionState.Commands.Add([System.Management.Automation.Runspaces.SessionStateFunctionEntry]::new("Parse-DbxFileName", $parseFuncDef))
    
    # Create the RunspacePool using the custom session state
    ...
    $scriptBlock = {
        param(
            ...
            [int]$jobIndex,
            [int]$totalFiles,
            [string]$PasswordFor7z
        )
        
        # Create file object
        $toplog_DBX_Item = Get-Item -Path $DBXFilePath
        
        Write-Host "`nProcessing file $jobIndex/$totalFiles`: $($toplog_DBX_Item.Name)" -ForegroundColor Cyan
        
        # Parse filename to get SN and date/time using the refactored function
        $fileInfo = Parse-DbxFileName -FileName $toplog_DBX_Item.Name
        if ($null -eq $fileInfo) {
            Write-Host "Error parsing filename in thread: $($toplog_DBX_Item.Name), skipping..." -ForegroundColor Red
            return
        }
        
        $sn = $fileInfo.SN
        $dateTime = $fileInfo.DateTime # Using the date part extracted by the function
        ...
        try {
            ...
            # Display processing information
            Write-Host "Extracting from $($toplog_DBX_Item.Name), SN: $sn" -ForegroundColor Yellow
            
            # Execute extraction - Re-added the password parameter using the variable
            $7zArgs = @("e", $toplog_DBX_Item.FullName) 
            if ($PasswordFor7z -ne '') {
                $7zArgs += "-p$PasswordFor7z"
            }
            $7zArgs += "-o$tempPath", "-aos"
            $7zArgs += $extractParams # Add the include filters
            ...
            Write-Host "Error processing $($toplog_DBX_Item.FullName): $_" -ForegroundColor Red
    ...
    foreach ($toplog_DBX in $toplog_DBXs) {
        ...
        [void]$powershell.AddParameter("PasswordFor7z", $ZipPassword) # Pass the password variable
